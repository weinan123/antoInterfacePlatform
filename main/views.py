...
def getProjectList(request):
    ...
    resp2 = moduleList.objects.values("moduleName", "owningListID")
    respList = list(resp2)
    for i in range(0, len(respList)):
        owningListID = respList[i]['owningListID']
        respList[i]['projectName'] = \
            projectList.objects.filter(id=owningListID).values("projectName")[0]['projectName']
    ...
    for j in range(0, len(respList)):
        returnData["model_list"].append(respList[j])
    # print returnData
    return JsonResponse(returnData, safe=False)
